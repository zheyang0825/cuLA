...
__device__ __forceinline__ void cp_async_8(void* smem, const void* global) {
    uint32_t sa = __cvta_generic_to_shared(smem);
...
__device__ __forceinline__ float4 load_bf16x4(const __nv_bfloat16* p) {
    ...
    *reinterpret_cast<uint2*>(tmp) = *reinterpret_cast<const uint2*>(p);
    return {__bfloat162float(tmp[0]), __bfloat162float(tmp[1]),
...
__device__ __forceinline__ void st_global_cg_u32(void* addr, uint32_t val) {
    asm volatile("st.global.cg.u32 [%0], %1;\n" :: "l"(addr), "r"(val));
}
__device__ __forceinline__ void st_global_cg_f32(void* addr, float val) {
    asm volatile("st.global.cg.f32 [%0], %1;\n" :: "l"(addr), "f"(val));
}
__device__ __forceinline__ void st_global_cg_f32x2(void* addr, float v0, float v1) {
    asm volatile("st.global.cg.v2.f32 [%0], {%1, %2};\n" :: "l"(addr), "f"(v0), "f"(v1));
}

__device__ __forceinline__ void mma_m16n8k8_acc(
...
__device__ __forceinline__ void matmul_1warp_2A_from_cache(
    float acc1[], float acc2[],
    const float cacheA1[][BT_S],
    const float cacheA2[][BT_S],
    int row_off, int col_off,
    const float B[][BK_S],
    int gid, int tid_in_grp,
    bool apply_mask, int sub_seq_len)
{
    float a1[8], a2[8];
    #pragma unroll
    ...
    for (int nt = 0; nt < NT; nt++) {
        ...
        for (int kk = 0; kk < 2; kk++) {
            ...
            mma_m16n8k8_acc(
                acc1[nt*4], acc1[nt*4+1], acc1[nt*4+2], acc1[nt*4+3],
                ...
                ub0, ub1);
            mma_m16n8k8_acc(
                acc2[nt*4], acc2[nt*4+1], acc2[nt*4+2], acc2[nt*4+3],
...
__device__ __forceinline__ void matmul_1warp_2B_transA_from_cache(
    float acc[],
    const float cacheA1[][BT_S],
    const float cacheA2[][BT_S],
    int row_off, int col_off,
...
{
    {
        ...
        for (int nt = 0; nt < NT; nt++) {
            ...
            for (int kk = 0; kk < 2; kk++) {
                ...
                mma_m16n8k8_acc(
                    acc[nt*4], acc[nt*4+1], acc[nt*4+2], acc[nt*4+3],
...
__device__ __forceinline__ void load_block_cp_async(
    float dst[][BK_S],
    const float* src, int row_base, int stride,
    int tile_seq_len, int tid)
{
    #pragma unroll
...
kda_bwd_intra_sm90_kernel(const KDA_bwd_intra_params params) {
    ...
    if (threadIdx.x < BT) {
        smem->beta_s[threadIdx.x] = (threadIdx.x < tile_seq_len) ?
            beta_base[threadIdx.x * stride_b] : 0.0f;
    }
    ...
    {
        const __nv_bfloat16 *q_base = (const __nv_bfloat16*)params.q_ptr + tile_offset * stride_qk + i_h * K + k_off;
        const __nv_bfloat16 *k_base = (const __nv_bfloat16*)params.k_ptr + tile_offset * stride_qk + i_h * K + k_off;
    ...
    if (i_i > 0) {
        ...
        for (int j = 0; j < i_i; j++) {
            ...
            __syncwarp();
            matmul_1warp_2A_from_cache(dq2, dk2,
                smem->dAqk_cache, smem->dAkk_cache,
                i_i * BC, j * BC, ww.B_a, gid, tid_in_grp,
                false, BC);
        }
        #pragma unroll
        for (int nt = 0; nt < NT; nt++) {
            ...
            dq2[nt*4+0] *= s00; dq2[nt*4+1] *= s01;
            dq2[nt*4+2] *= s10; dq2[nt*4+3] *= s11;
            dk2[nt*4+0] *= s00; dk2[nt*4+1] *= s01;
            dk2[nt*4+2] *= s10; dk2[nt*4+3] *= s11;
        }
    }
    ...
    // ==================== FORWARD DIAGONAL ====================
    {
        ...
        float dqd[16] = {0};
        float dkd[16] = {0};
        matmul_1warp_2A_from_cache(dqd, dkd,
            smem->dAqk_cache, smem->dAkk_cache,
            i_i * BC, i_i * BC, ww.B_a, gid, tid_in_grp,
            true, sub_seq_len);
        ...
        for (int nt = 0; nt < NT; nt++) {
            ...
            float s10 = exp2f(my_g[row1][col0] - gn0);
            float s11 = exp2f(my_g[row1][col1] - gn1);
            dq2[nt*4+0] += dqd[nt*4+0] * s00; dq2[nt*4+1] += dqd[nt*4+1] * s01;
            dq2[nt*4+2] += dqd[nt*4+2] * s10; dq2[nt*4+3] += dqd[nt*4+3] * s11;
            dk2[nt*4+0] += dkd[nt*4+0] * s00; dk2[nt*4+1] += dkd[nt*4+1] * s01;
            dk2[nt*4+2] += dkd[nt*4+2] * s10; dk2[nt*4+3] += dkd[nt*4+3] * s11;
        }
    }
    ...
    // Write dq_out = dq2 + dq_in, then reuse dq2 for dg_p = q * dq2
    {
        ...
        for (int nt = 0; nt < NT; nt++) {
            int col0 = nt * 8 + tid_in_grp * 2;
            if (row0 < sub_seq_len) {
                __nv_bfloat162 pair = {__float2bfloat16(dq2[nt*4+0] + ww.B_a[row0][col0]),
                                       __float2bfloat16(dq2[nt*4+1] + ww.B_a[row0][col0+1])};
                st_global_cg_u32(&dq_out[tile_r0 * stride_qk + col0],
                    *reinterpret_cast<uint32_t*>(&pair));
            }
            if (row1 < sub_seq_len) {
                __nv_bfloat162 pair = {__float2bfloat16(dq2[nt*4+2] + ww.B_a[row1][col0]),
                                       __float2bfloat16(dq2[nt*4+3] + ww.B_a[row1][col0+1])};
                st_global_cg_u32(&dq_out[tile_r1 * stride_qk + col0],
                    *reinterpret_cast<uint32_t*>(&pair));
            }
    ...
    // Scale dk2 by beta
    {
        ...
        #pragma unroll
        for (int nt = 0; nt < NT; nt++) {
            dk2[nt*4+0] *= beta0; dk2[nt*4+1] *= beta0;
            dk2[nt*4+2] *= beta1; dk2[nt*4+3] *= beta1;
        }
    }
    ...
    if (i_i < NC_actual - 1) {
        ...
        for (int j = i_i + 1; j < NC_actual; j++) {
            ...
            __syncwarp();
            matmul_1warp_2B_transA_from_cache(dkt,
                smem->dAqk_cache, smem->dAkk_cache,
                j * BC, i_i * BC, ww.B_a, ww.B_b, gid, tid_in_grp,
                false, BC);
        }
        #pragma unroll
        for (int nt = 0; nt < NT; nt++) {
            ...
            dkt[nt*4+0] *= s00; dkt[nt*4+1] *= s01;
            dkt[nt*4+2] *= s10; dkt[nt*4+3] *= s11;
        }
    }
    ...
    // ==================== BACKWARD DIAGONAL ====================
    {
        ...
        float dktd[16] = {0};
        matmul_1warp_2B_transA_from_cache(dktd,
            smem->dAqk_cache, smem->dAkk_cache,
            i_i * BC, i_i * BC, ww.B_a, ww.B_b, gid, tid_in_grp,
            true, sub_seq_len);
        ...
        for (int nt = 0; nt < NT; nt++) {
            ...
            float s10 = exp2f(gn0 - my_g[row1][col0]);
            float s11 = exp2f(gn1 - my_g[row1][col1]);
            dkt[nt*4+0] += dktd[nt*4+0] * s00; dkt[nt*4+1] += dktd[nt*4+1] * s01;
            dkt[nt*4+2] += dktd[nt*4+2] * s10; dkt[nt*4+3] += dktd[nt*4+3] * s11;
        }
    }
...

    {
        ...
        for (int nt = 0; nt < NT; nt++) {
            ...
            if (row0 < sub_seq_len) {
                ...
                __nv_bfloat162 dk_pair = {__float2bfloat16(ww.B_a[row0][col0] + dk2[nt*4+0] + dkt[nt*4+0]),
                                          __float2bfloat16(ww.B_a[row0][col0+1] + dk2[nt*4+1] + dkt[nt*4+1])};
                st_global_cg_u32(&dk_out[off0], *reinterpret_cast<uint32_t*>(&dk_pair));
                st_global_cg_f32x2(&dg_out[off0],
                    dq2[nt*4+0] + (dk2[nt*4+0] - dkt[nt*4+0]) * bf2f(my_k[row0][col0]) + ww.B_b[row0][col0],
                    dq2[nt*4+1] + (dk2[nt*4+1] - dkt[nt*4+1]) * bf2f(my_k[row0][col0+1]) + ww.B_b[row0][col0+1]);
            }
            if (row1 < sub_seq_len) {
                int off0 = tile_r1 * stride_qk + col0;
                __nv_bfloat162 dk_pair = {__float2bfloat16(ww.B_a[row1][col0] + dk2[nt*4+2] + dkt[nt*4+2]),
                                          __float2bfloat16(ww.B_a[row1][col0+1] + dk2[nt*4+3] + dkt[nt*4+3])};
                st_global_cg_u32(&dk_out[off0], *reinterpret_cast<uint32_t*>(&dk_pair));
                st_global_cg_f32x2(&dg_out[off0],
                    dq2[nt*4+2] + (dk2[nt*4+2] - dkt[nt*4+2]) * bf2f(my_k[row1][col0]) + ww.B_b[row1][col0],
                    dq2[nt*4+3] + (dk2[nt*4+3] - dkt[nt*4+3]) * bf2f(my_k[row1][col0+1]) + ww.B_b[row1][col0+1]);
            }
        }
    }
...
void run_kda_bwd_intra_sm90(KDA_bwd_intra_params &params, cudaStream_t stream) {
    ...
    cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &num_blocks_per_sm, kernel, BLOCK_THREADS, smem_size);
