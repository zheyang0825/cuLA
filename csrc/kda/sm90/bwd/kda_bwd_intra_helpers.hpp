...
setup_kg_intra_offdiag_gmem(SKG& sKG, GK const& gK_j, GG const& gG_j, const float* s_gn, int tid) {
...
__device__ __forceinline__ void
setup_kg_intra_diag(SKG& sKG, SK const& sK, SG const& sG, const float* s_gn, int T_seq, int i_ti, int tid) {
...
// 1 thread = 1 (k_dim, n..n+3) chunk.
// ──────────────────────────────────────────────────────────────────
template <int BC, int BK, int NUM_THREADS, class GQ, class GK, class GG, class GBETA, class SQG, class SKBG>
__device__ __forceinline__ void
...
// 1 thread = 1 element.
// ──────────────────────────────────────────────────────────────────
template <int BC, int BK, int NUM_THREADS, class SQ, class SK, class SG, class SQG, class SKBG>
__device__ __forceinline__ void
...
__device__ __forceinline__ void
epilogue_output_dq(
    GDq const& gDq_tile, GDqOut& gDqOut_tile, SStage& sStage, const float dq_acc[4], int T_seq, int i_ti, int tid) {
...
// Caller must __syncthreads() before and after this helper.
// ──────────────────────────────────────────────────────────────────
template <int BC, int BK, int NUM_THREADS, class SQ, class SK, class GDg, class GDgOut, class SStage>
__device__ __forceinline__ void
epilogue_output_dg(
    SQ const& sQ,
    SK const& sK,
    ...
    GDgOut& gDgOut_tile,
    SStage& sStage,
    const float dq_acc[4],
    const float dk_acc[4],
    const float dkt_acc[4],
    int T_seq,
    int i_ti,
    bool is_boundary,
    int tid) {
