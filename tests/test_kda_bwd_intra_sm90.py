    ...
    for d_start in range(0, D, K_TILE):
        ...
        for is_ in range(num_sub):
            for js in range(is_ + 1):
                ...
                A_sub = dAqk[is_ * SUB_T:(is_ + 1) * SUB_T,
                             js * SUB_T:(js + 1) * SUB_T].clone()
                if is_ == js:
        ...
        for is_ in range(num_sub):
            for js in range(is_ + 1):
                ...
                A_sub = dAkk[is_ * SUB_T:(is_ + 1) * SUB_T,
                             js * SUB_T:(js + 1) * SUB_T].clone()
                if is_ == js:
        ...
        for js in range(num_sub):
            for is_ in range(js, num_sub):
                ...
                k_scale = torch.exp2(torch.clamp(
                    g_norm - g_slice[js * SUB_T:(js + 1) * SUB_T, :],
                    min=-126.0, max=126.0))
        ...
        for js in range(num_sub):
            for is_ in range(js, num_sub):
                g_norm = g_slice[is_ * SUB_T, :]
                KBG_is = (k_slice[is_ * SUB_T:(is_ + 1) * SUB_T, :]
    ...
    for b_idx in range(B):
        for c in range(num_chunks):
            for h in range(H):
                ...
                dq_ref, dk_ref = compute_dq_dk_intra_ref(
                    q_ch, k_ch, g_ch, beta_ch, dAqk_ch, dAkk_ch)

                dq_cuda = dq_out[start:end, h, :]  # [64, D]
                dk_cuda = dk_out[start:end, h, :]  # [64, D]

                torch.testing.assert_close(
                    dq_cuda, dq_ref, atol=5e-3, rtol=1e-3,
                    msg=f"dQ mismatch: b={b_idx}, c={c}, h={h}",
                )
                torch.testing.assert_close(
                    dk_cuda, dk_ref, atol=5e-3, rtol=1e-3,
    ...
    for b_idx in range(B):
        for c in range(num_chunks):
            for h in range(H):
                ...
                dq_gt, dk_gt = compute_dq_dk_intra_ref_gnorm(
                    q_ch, k_ch, g_ch, beta_ch, dAqk_ch, dAkk_ch)
                ...
                torch.testing.assert_close(
                    dq_cuda, dq_gt, atol=5e-3, rtol=1e-3,
                    msg=f"dQ ref mismatch: b={b_idx}, c={c}, h={h}",
                )
                torch.testing.assert_close(
                    dk_cuda, dk_gt, atol=5e-3, rtol=1e-3,
